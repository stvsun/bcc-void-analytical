...
The most consequential difference is the activation pressure. The BCC
value $p^* = 3\sqrt{6}/4\,\tcrss$ exceeds the FCC value
$p^* = \sqrt{6}/2\,\tcrss$ by a factor of \emph{exactly}~$3/2$:
\begin{equation}
\label{eq:ratio_section}
\frac{p^*_{\mathrm{BCC}}}{p^*_{\mathrm{FCC}}}
  = \frac{3\sqrt{6}/4}{\sqrt{6}/2} = \frac{3}{2}.
\end{equation}
This ratio is a purely geometric consequence of the yield polygon
dimensions and is independent of the elastic constants, the void
radius, or the applied loading magnitude. The 50\% higher activation
pressure means that, for the same critical resolved shear stress, a BCC
crystal requires 50\% more far-field stress to sustain plastic flow
around a void than an FCC crystal. This provides a quantitative,
single-crystal-level explanation for the long-observed superior
resistance of BCC ferritic steels to radiation-induced void
swelling~\citep{garner2010, zinkle2013}.
...
$\theta$-dependent rigid-plastic prediction.
